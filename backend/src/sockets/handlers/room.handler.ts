  ...
  socket.on('room:join', async (payload: RoomJoinPayload, ack) => {
    ...
    try {
      ...
      if (!roomBuffers.has(roomCode)) {
        ...
        if (currentSlide) {
          const targetRegions = currentSlide.type === 'spatial-hotspot' ? currentSlide.targetRegions : undefined;
          const answerCards = currentSlide.type === 'peer-review-swarm' ? currentSlide.answerCards : undefined;
          const phase = currentSlide.type === 'peer-review-swarm' ? currentSlide.currentPhase : undefined;
          buffer.initSlide(currentSlide.id, currentSlide.type as any, (currentSlide as any).conceptTags ?? [], targetRegions, answerCards, phase);
        }
        roomBuffers.set(roomCode, buffer);
        flushScheduler.start(roomCode, buffer, io);
      }

      // If reconnecting participant during grading phase, re-send their assigned cards
      const activeBuffer = roomBuffers.get(roomCode);
      if (currentSlide?.type === 'peer-review-swarm' && socket.data.role === 'participant' && activeBuffer) {
        const assignedIds = activeBuffer.getPeerReviewAssignments(currentSlide.id, socket.data.participantId);
        if (assignedIds && assignedIds.length > 0) {
          const allAnswers = activeBuffer.getAllPeerReviewAnswers(currentSlide.id);
          const cards = allAnswers
            .filter((a) => assignedIds.includes(a.id))
            .map((a) => ({ id: a.id, content: a.content }));
          socket.emit('review:assigned_cards', { slideId: currentSlide.id, cards });
        }
      }
