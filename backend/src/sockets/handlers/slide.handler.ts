...
export function registerSlideHandlers(
  ...
  socket.on('slide:advance', (payload: SlideAdvancePayload) => {
    ...
    try {
      ...
      if (buffer) {
        const targetRegions = newSlide.type === 'spatial-hotspot' ? newSlide.targetRegions : undefined;
        const answerCards = newSlide.type === 'peer-review-swarm' ? newSlide.answerCards : undefined;
        const phase = newSlide.type === 'peer-review-swarm' ? newSlide.currentPhase : undefined;
        buffer.initSlide(newSlide.id, newSlide.type as any, (newSlide as any).conceptTags ?? [], targetRegions, answerCards, phase);
      }
