import type { NetworkMatchingSlide } from '@pairly/schemas';
interface Props { slide: NetworkMatchingSlide }
export function PresenterGraph({ slide }: Props) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', padding: 'var(--space-6)' }}>
      ...
      <div style={{ flex: 1, background: 'var(--color-bg-elevated)', borderRadius: 'var(--radius-xl)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--color-border-subtle)' }}>
        <span style={{ color: 'var(--color-text-tertiary)', fontSize: 'var(--text-sm)' }}>D3 Force-Layout Bipartite Graph — renders as participants connect edges</span>
      </div>
    </div>
  );
}
